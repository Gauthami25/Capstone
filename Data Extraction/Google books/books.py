    ...
    def reformat_search():
        for item in json_response_books:
            ...
            try:
                ...
                # publisher = item["volumeInfo"]['description']
            except KeyError:
            ...
            API_reading_list_alt = {"TITLE" : title, "AUTHOR": author, "PUBLISHER": publisher, "DESCRIPTION": description}
            # ... And then add all these new books into our empty "combine_dict".
            combine_dict.append(API_reading_list)
            combine_dict_alt.append(API_reading_list_alt)

            # print(f'jsut checking----> {combine}')
    reformat_search()
    my_df = pd.DataFrame(combine_dict_alt)
    # my_df.to_csv("Books_on_climate_change.csv")
